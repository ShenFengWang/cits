<?php if (!defined('THINK_PATH')) exit();?><div id="memberadmin_accordion" class="easyui-accordion">
	<div title="正式成员">
		<div class="easyui-layout" data-options="fit:true,border:false">
<?php if($loginstatus > 2): ?><div region="west" style="padding:10px;width:200px;">
				<ul id="memberadmin_menu"></ul>
			</div><?php endif; ?>
			<div region="center">
				<div id="memberadmin_formal">
					<div id="memberadmin_formal_toolbar" style="display:none;">
<?php if($loginstatus >= $EXTRA_memberadmin): ?><a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true" onclick="memberadmin_remove();">删除</a><?php endif; ?>
...
<?php if($loginstatus >= $EXTRA_memberadmin): ?><div title="未授权成员" <?php echo ($unsign); ?>>
...
$('#memberadmin_unsign').datagrid({
	url:'/webforcits/home/management/memberadminJson/unsign',
	method:'get',
	striped:true,
	singleSelect:true,
	fit:true,
	...
	columns:[[
		...
		{field:'authority',title:'基础权限',width:120,editor:{
			...
			url:'/webforcits/home/management/memberadminJson/authority',required:true,missingMessage:'此项必选',editable:false
		}}},
		{field:'actor',title:'角色',width:120,editor:{
			type:'combobox',options:{valueField:'id',textField:'name',method:'get',
			url:'/webforcits/home/management/memberadminJson/actor',required:true,missingMessage:'此项必选',editable:false,
			onChange:function(value){
				var ingroup = $('#memberadmin_unsign').datagrid('getEditor',{index:memberadmin_unsign_editIndex,field:'ingroup'}).target;
				ingroup.combobox({
					valueField:'id',textField:'name',url:'/webforcits/home/management/memberadminJson/ingroup/' + value,
...
function memberadmin_unsign_ajax(id,data,action){
	$.post('/webforcits/home/management/memberadmin',{id:id,data:data,action:action,target:'unsign',identify:identify})
...
<?php if($loginstatus > 2): ?>//组列表菜单
$('#memberadmin_menu').tree({
	url:'/webforcits/home/management/memberadminJson/menu',
	method:'get',
	animate:true,
	border:false,
...
<?php if($loginstatus == 2): ?>//组列表内容
memberadmin_formal();<?php endif; ?>

//组列表内容生成
function memberadmin_formal(id){
	$('#memberadmin_formal').datagrid({
		url:'/webforcits/home/management/memberadminJson/formal/' + id,
		method:'get',
		striped:true,
		singleSelect:true,
		fit:true,
		...
		{field:'actor_name',title:'角色',width:120,editor:{
			...
			onLoadSuccess:function(){
				...
				ingroup.combobox({
					valueField:'id',textField:'name',url:'/webforcits/home/management/memberadminJson/ingroup/' + $('#memberadmin_formal').datagrid('getSelected').loginstatus,
					onLoadSuccess:function(){
...
function memberadmin_ajax(id,data,action){
	$.post('/webforcits/home/management/memberadmin',{id:id,data:data,action:action,target:'formal',identify:identify})
	.success(function(callback){
		if(callback != 1){
			$.messager.show({title:'提示',msg:callback});
		}
...
<?php if($loginstatus >= $EXTRA_memberadmin): ?>//删除

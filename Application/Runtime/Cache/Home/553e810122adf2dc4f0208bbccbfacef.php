<?php if (!defined('THINK_PATH')) exit();?><div id="route_table">
	<div id="route_toolbar" style="display:none;">
<?php if($loginstatus >= $EXTRA_route): ?><a href="javascript:void(0)" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-add'" onClick="route_add()">增加</a>
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-cancel'" onClick="route_cancel()">取消</a><?php endif; ?>
...
$('#route_table').datagrid({
	url:'/webforcits/home/management/routeJson/travel',
	method:'get',
	striped:true,
	singleSelect:true,
	...
	columns:[[
		...
		valueField:'id',textField:'name',url:'/webforcits/home/management/routeJson/province',method:'get',onLoadSuccess:function(){
...
<?php if($loginstatus >= $EXTRA_route): ?>//增加
function route_add(){
	if(route_endEdit()){
		$('#route_table').datagrid('cancelEdit',route_editIndex);
...
function route_ajax(id,data,action){
	$.post('/webforcits/home/management/route',{id:id,data:data,action:action,identify:identify})
